...
class RaySampleInterval(torch.nn.Module):
    def __init__(self, cfg, point_amount = 64, lindisp = True, perturb = False):
        ...
        point_intervals = torch.linspace(0.0, 1.0, point_amount)[None, :]
    ...
    def forward(self, near, far):
        ...
        point_intervals = point_intervals.expand([ self.cfg.nerf.train.num_random_rays, self.point_amount ])
        if self.perturb:
