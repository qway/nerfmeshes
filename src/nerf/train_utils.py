...
def predict_and_render_radiance(
    ...
    if depth_values is None or mode is not "train":
        ...
        z_vals = MultivariateNormal(torch.zeros((num_rays, )).to(ro.device), torch.eye(num_rays).to(ro.device) * variance).sample((total, )).transpose(1, 0)

        if use_stacking:
            # stack 2-halfs, stronger sampling at peak
            if use_truncated:
                shift = -threshold * torch.tensor(variance).sqrt()
                mask = z_vals < shift
            ...
            maxx = z_vals.max(1).values[:, None]
            minx = z_vals.min(1).values[:, None]

            if use_truncated:
                z_vals = 1 - z_vals / maxx
            else:
                z_vals = 1 - (z_vals - minx) / torch.max(torch.ones_like(maxx), maxx - minx)
        else:
            minx = z_vals.min(1).values[:, None]

            z_vals = (z_vals - minx) / abs(minx)

        if use_truncated:
            z_vals = torch.cat((z_vals, torch.ones((num_rays, 1), device = ro.device)), dim = -1)
    ...
    # return None, None, None, None, rgb_fine, disp_fine, acc_fine, depth_fine

    return rgb_coarse, disp_coarse, acc_coarse, depth_coarse, rgb_fine, disp_fine, acc_fine, depth_fine
    # return rgb_coarse, disp_coarse, acc_coarse, depth_coarse, rgb_fine, disp_fine, acc_fine, depth_fine, z_samples_all, torch.cat((weights_1, weights_2), dim=-1)
